...
public interface EmailMailboxEntryRepository extends JpaRepository<EmailMailboxEntry, EmailMailboxEntryId> {
    ...
    @Query(value = """
        ...
        join fetch m.sender
        left join m.account account
        where e.user.userId = :userId
          and (account is null or account.status <> com.example.Email.Entity.EmailAccountStatus.DISCONNECTED)
          and (
          ...
          and (:accountId is null or account.accountId = :accountId)
        ...
        """,
        countQuery = """
        select count(e) from EmailMailboxEntry e join e.message m left join m.account account
        where e.user.userId = :userId
          and (account is null or account.status <> com.example.Email.Entity.EmailAccountStatus.DISCONNECTED)
          and (
            (:folder = 'INBOX' and e.mailboxRole = com.example.Email.Entity.EmailMailboxRole.RECIPIENT and m.status = com.example.Email.Entity.EmailMessageStatus.SENT and e.archived = false and e.spam = false and e.trashedAt is null)
    ...
    @Query("""
      ...
      from EmailMailboxEntry e join e.message m left join m.account account where e.user.userId = :userId
        and (account is null or account.status <> com.example.Email.Entity.EmailAccountStatus.DISCONNECTED)
      """)
