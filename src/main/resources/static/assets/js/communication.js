(() => {
  ...
  async function localAuth(path, body) { const response=await fetch(path,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});const data=await response.json().catch(()=>({}));if(!response.ok){const fieldMessage=data.validationErrors&&Object.values(data.validationErrors).filter(Boolean).join(' ');throw new Error(fieldMessage||data.message||(path.endsWith('/register')?'Unable to create the account. Please check the information and try again.':'Unable to sign in. Please check your email and password.'));}localStorage.setItem('crmsSession',JSON.stringify({access_token:data.accessToken,user:data.user}));return JSON.parse(localStorage.getItem('crmsSession')); }
  ...
  $('signUpBtn').onclick = async () => { try { const email=normalizeEmail($('loginEmail').value);const password=$('loginPassword').value;const fullName=$('loginName').value.trim();if(!fullName)throw new Error('Full name is required.');if(!email)throw new Error('Email is required.');if(!/^(?=.*[A-Za-z])(?=.*\d)(?=.*[^A-Za-z0-9\s])\S{8,72}$/.test(password))throw new Error('Password must be 8–72 characters and include at least one letter, one number, and one special character. Spaces are not allowed.');await boot(await localAuth('/api/auth/register',{fullName,email,password})); authModal.hide(); } catch(e) { $('authError').textContent=e.message; } };
